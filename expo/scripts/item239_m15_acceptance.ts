 ...
 *
 * (a) map at 2026-08-27T02:15Z must contain a RESISTANCE-behaving zone in 4633-4641;
 * (b) map at 2026-08-27T10:37Z must contain a SUPPORT-behaving zone in 4559-4572;
...
async function main(): Promise<void> {
  ...
  const a = m1?.zones.find(z => z.role === 'RESISTANCE' && z.lo <= 4641 && z.hi >= 4633) ?? null;
  console.log(`  (a) RESISTANCE-behaving zone overlapping 4633-4641: ${a ? `YES -> ${a.lo.toFixed(1)}-${a.hi.toFixed(1)} n=${a.n} rb=${a.rb} ra=${a.ra}` : 'NO -> FAIL'}`);
  ...
  const s = m2?.zones.find(z => z.role === 'SUPPORT' && z.lo <= 4572 && z.hi >= 4559) ?? null;
  console.log(`  (b) SUPPORT-behaving zone overlapping 4559-4572: ${s ? `YES -> ${s.lo.toFixed(1)}-${s.hi.toFixed(1)} n=${s.n} rb=${s.rb} ra=${s.ra}` : 'NO -> FAIL'}`);
