...
export async function hydrateFromRemote(options?: { limit?: number; cap?: number }): Promise<{
  ...
  persistCorpusStats(true);
  console.log(`🔄 [LearningStore] Remote hydrate (direct Supabase, ${read.pages} page(s)): pulled ${remote.length}, merged ${merged} new, backfilled ${backfilled} local-only, total ${total}`);
