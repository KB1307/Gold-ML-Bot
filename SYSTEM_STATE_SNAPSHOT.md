...
## G-CHRONOLOGY-2 (Correction 5) — 2026-08-15

**G-CHRONOLOGY-2 — PASS.** The previous G-CHRONOLOGY report's conclusion ("The 0.22pp value-distribution residual IS CONSISTENT with the 0.07pp rejection-count residual... it is NOT 3x the noise floor") is **RETRACTED**. A fresh pair of harness runs (same code, same tape, same --step 10, same --tape-end 1786776930000, 13,830 bars, 1353 attempts, n=1347 each, TIER0_UNAVAILABLE=0) were diffed index-for-index using the unsorted (tape-order) WS_UNSORTED vector (added to the harness at line 430 for this purpose). Results: **43 of 1347 evaluations differ (3.19%), and 22 of those 43 CROSSED the 0.55 conviction threshold.** RUN1 pass=472 (35.0%), RUN2 pass=458 (34.0%), net rejection-count residual=14 (1.04%). The mechanism proposed ("within-bucket value shifts are invisible to binary threshold-crossing counts") is WRONG — more than half of the shifted evaluations DID cross 0.55. The 0.07pp residual Item 72 established was at --step 25 (harness default), not --step 10; the actual determinism gap at --step 10 is much larger (1.04% rejection-count, 3.19% value-distribution). The 117-vs-120 discrepancy at 0.5300 between Item 73 and G80-1-CORRECTED (0.22pp) is well within this larger noise floor and remains a determinism gap, not a code effect. PATH TAKEN: PATH A with fresh vectors (not the original pair, which are not recoverable — not saved to /tmp, not committed, conversation context cleared, and the WS_VECTOR is SORTED not unsorted so index-for-index diffing was impossible even if recovered). Fresh pair produced instead with identical parameters, harness modified to output WS_UNSORTED (tape-order) vector.
