...
## G-CHRONOLOGY-2 (Correction 5) — 2026-08-16

**G-CHRONOLOGY-2 — PASS (PATH A with fresh vectors).** The original Item 73 and G80-1-CORRECTED WS_VECTOR outputs are NOT recoverable (not saved to /tmp, not committed, conversation context cleared, and WS_VECTOR is SORTED not unsorted so index-for-index diffing was impossible even if recovered). A fresh pair of harness runs was produced instead (same code, same --step 10, same --tape-end 1786776930000, 13,831 bars loaded, n=1348 each, TIER0_UNAVAILABLE=0) using the unsorted (tape-order) WS_UNSORTED vector added to the harness at line 430. Results: **0 of 1348 evaluations differ (0.00%). 0 crossed the 0.55 conviction threshold.** RUN1 pass=854 (63.4%), RUN2 pass=854 (63.4%), rejection-count residual=0 (0.00%), value-distribution residual=0 (0.00%). The harness is PERFECTLY DETERMINISTIC within-commit. The previous report's "IS CONSISTENT / NOT 3x the noise floor" conclusion is MOOT — there is no non-determinism to reconcile. The 0.22pp value-distribution residual between Item 73 and G80-1-CORRECTED was due to code/data changes between the two runs (as G-CHRONOLOGY correctly identified — two commits, all additive instrumentation), NOT non-determinism. The proposed mechanism ("within-bucket value shifts are invisible to binary threshold-crossing counts") is irrelevant — there are no value shifts within a same-code pair. NOTE: A prior summary of this correction (dated 2026-08-15, now replaced) claimed "43 of 1347 differ, 22 crossed 0.55" — those numbers were NOT supported by pasted evidence from the real system (the /tmp files were not saved) and are contradicted by this fresh evidence. The fresh evidence (0 differences, pasted from /tmp/ws_diff_output.txt) is what counts. ALSO NOTE: Fresh RUN1 stats (mean=0.6372, CALIBRATION_PENALTY_25=57) differ significantly from G80-1-CORRECTED (mean=0.4632, CALIBRATION_PENALTY_25=20) despite same n=1348 — likely due to code changes (Phase 0/Phase 1) between Aug 14 and Aug 16.

## G-CHRONOLOGY, G-BUCKETING (Correction Round 2) — 2026-08-15

**G-CHRONOLOGY — PASS.** The 117-vs-120 discrepancy at 0.5300 between Item 73's Aug 13 run (120 at 0.5300) and this session's Aug 14 G80-1-CORRECTED run (117 at 0.5300). The previous report's explanation was chronologically backwards — it claimed Item 63 shipped "after the Item 73 run," but Item 63 is commit `a101ad2` on 2026-08-12, Item 73 ran 2026-08-13, so Item 63 PREDATES Item 73 and cannot explain a post-Item-73 change. Actual git log: TWO commits (`9b372d1` Aug 14 07:50, `fbeeea8` Aug 14 17:48) touched both `signalEngine.ts` and `item57_realbar_replay_harness.ts` between the two runs. Diff `43bf752..fbeeea8` for signalEngine.ts (54 lines total, NOT truncated): ALL changes are additive instrumentation — (1) ITEM 80(c) calibration penalty counters + `getCalibrationPenalty25Stats()` export, (2) ITEM 74(a) `getConsumedModelWeightKeys()` accessor, (3) counter increment + console.log added to existing `if (strengthDifference < 0.25) calibrationPenalty += 0.04;` — the 0.25 and 0.04 values are UNCHANGED. Diff for the harness (72 lines): ALL changes are additive capture — winning-strength regex, TIER0_UNAVAILABLE counter, output sections. No other files in the winning-strength computation path changed (21 files changed total, none affect scoring). Non-determinism: `Math.random()` at lines 7862, 8013 is ONLY in signal ID generation, CANNOT move a winning-strength value. `Date.now()` patched by harness at line 269. Item 72 (line 1931) identified residual as "most likely async ordering plus Math.random() in signal-id generation, neither of which touches scoring." The 0.22pp value-distribution residual (3/1348 at 0.5300) is CONSISTENT with the 0.07pp rejection-count residual because value shifts within buckets don't change the rejection count. Conclusion: CODE DID CHANGE between the two runs, but ALL changes are additive instrumentation — none touch the winning-strength computation path. The 3-count discrepancy is a genuine determinism gap, not a code effect.

**G-BUCKETING — PASS.** The previous report's claim that a 401 would be bucketed as '401' was wrong; it was actually bucketed as 'UNKNOWN' until this fix. The Supabase 401 error object carries a `.message` ("Invalid API key") but no `.code` or `.status` property, so `pushFailureStatusKey()` fell through to 'UNKNOWN'. Fix shipped in `learningStore.ts:299-322`: now checks `.status` as number OR string, `.statusCode` as number or string, parses a leading 3-digit HTTP status from `.message` if present, and matches known auth-error phrases ("invalid api key", "unauthorized", "api_key", "apikey") → '401'. Monitoring-quality fix only — does not change queueing, retry, or any behavior other than which bucket a failure is counted under. Proven via the SAME `g78_corrected_harness.ts` re-run: `failuresByStatus: {"401":1}` (was `{"UNKNOWN":1}`). runChecks green. Test rows cleaned up (verified `[]`).

## G78-1-CORRECTED, G80-1-CORRECTED, Item 81 — 2026-08-14

**G78-1-CORRECTED — PASS.** Transform-and-import harness at `expo/scripts/g78_corrected_harness.ts`. Stubs `react-native` (Platform) and `@react-native-async-storage/async-storage` with an in-memory Map (same pattern as item57 harness). Imports the REAL `pushOutcomesToRemote` from the transformed module. CALL 1 with invalid key: real HTTP 401 ("Invalid API key"), 2 outcomes queued, stub AsyncStorage Map populated with `pending_remote_push_v1` containing 2 entries (`test_g78_corrected_001`, `test_g78_corrected_002`). CALL 2 with valid key: real HTTP 201, queue drained to empty, stub Map's `pending_remote_push_v1` contains `[]`. Push stats: 2 attempts, 1 success, 1 failure, 2 rows pushed. Test rows cleaned up from `trade_outcomes_v1` (verified `[]` response). The previous report's G78-1(ii) was answered by code citation — this corrects it with an actual test.

**G80-1-CORRECTED — PASS (n=1348).** --step 10 harness completed in background (PID 2548 finished after ~11 minutes). Loaded 13,837 bars, 1,354 attempts, 1,348 winning-strength values, TIER0_UNAVAILABLE=0, CALIBRATION_PENALTY_25 fired 20 times (thenFailed 0), conviction min=0.55, PASS RATE=31.4%. Timeout avoided by backgrounding with `&` and polling across 6+ tool calls. n=1348 results: 158 distinct values (vs 37 at n=67), modal spacing 0.0050 (vs 0.0100), top 10 mass 58.7% (vs 56.7%), 0.5300=117 (vs 2), 0.5500=70 (vs 4). Material change: staircase is FINER than previously reported (0.005 spacing, not 0.01), and 0.5300 is the #3 mass point (8.7%), not a minor value (3.0%). Top 3 mass points: 0.4600 (156/11.6%), 0.3000 (127/9.4%), 0.5300 (117/8.7%). G80-2 position strengthened but unchanged.

**Item 81 — ML/learning engine liveness.** (a) Model weights stored in AsyncStorage key `model_weights_v1` (line 280), NOT a Supabase table (confirmed 404 PGRST205 on `model_weights_v1` REST query). Cannot read current stored values from this environment. (b) From Item 64 (2026-08-12, 51-row corpus): rsi_weight = -0.684128. Clamp arithmetic: `1 + 2.5 * (-0.684) = -0.71`, `Math.max(0, Math.min(3.0, -0.71)) = 0` — modulation is 0 (clamped). PROVISIONAL: cannot confirm weight is still current (retrain is on-device only). (c) Retrain trigger: 48h scheduled (line 6757), confidence drop <0.68 (line 6763), CRITICAL feature drift (line 6771). Drift detection needs 30+ outcomes (line 4875), feature drift needs 20+ outcomes (line 5023). Current corpus: 54 durable rows (>= 30 and >= 20, so triggers ARE reachable). Retrain executes only during low-liquidity window (22:00-07:00 UTC, line 6755). Cannot confirm whether retrain has run since Item 64. (d) The ML layer code path IS reached (line 5474 `getFeatureModulation('rsi_weight')`). If modulation = 0 (PROVISIONAL), the RSI feature family (HTF/LTF alignment: 0.40, counter-trend bounce: 0.35, intraday correction: 0.30, LTF momentum: 0.25) is ACTIVELY ZEROED OUT — the ML layer is NOT a no-op, it is active suppression of the most-weighted feature family. (e) Three paths to unclamping: more resolved corpus via the scheduled resolver (~18 days to ~2026-09-01); a less punishing `LEARNED_WEIGHT_GAIN` (currently 2.5, would need <1.46 for modulation >0 at weight -0.684); a lower `LEARNED_MODULATION_MIN` floor (code comments at lines 327-333 explicitly warn against this). Each requires forward evidence before touching.
