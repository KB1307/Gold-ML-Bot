...
| **F-29** | **FIXED 2026-08-18 (ITEM 94).** Root cause isolated: `backend/functions/resolve-emitted-signals/index.ts` line 214 `lockPrice = entry` (breakeven) after TP1 instead of the 0.35R profit lock. When SL_AFTER_BE fired, `r = rOf(entry) = 0 - cost < 0` → `isWin()` returned LOSS. 73 rows backfilled (LOSS→WIN, realized_r 0/null/−1 → 0.27-0.33). Post-repair book: n=349 WR=60.17% EV=+0.1562R (was 39.26%/+0.0677R). Assertion in resolveFromBars: `if (r <= 0) throw` at the SL_AFTER_BE return path. | `backend/functions/resolve-emitted-signals/index.ts:214` (was `lockPrice = entry`, now `computePostTP1LockPrice(signal)`); `isWin` now includes `status === "SL_AFTER_BE"` | **CRITICAL → RESOLVED** | Pre-existing | HC-94a/94b/94c/94d/94e |
