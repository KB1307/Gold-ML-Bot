...
| **F-29** | **NEW (BLOCK A / A-4) — `trade_outcomes_v1.realized_r` OMITS THE 0.35R POST-TP1 LOCK FOR ~51% OF SL_AFTER_BE ROWS.** Of 144 canonical `SL_AFTER_BE` rows (TP1 banked, later retraced to the 0.35R lock — always a true WIN), 73 (50.7%) are stored as LOSS or `realized_r<=0`, i.e. the write path drops the banked profit and records a bare stop-out. This is the exact mechanism for the WR gap between the canonical book (62.8%) and `trade_outcomes_v1.realized_r` (39.9%). The write-path function that produces this corruption was not isolated to a single line this round — named as the next diagnostic action. | `trade_outcomes_v1` write path (TradingContext.tsx `recordTradeOutcome` call sites: 1743/1856/2122/2135/2148/3156) | **CRITICAL** — every WR/EV figure read from `trade_outcomes_v1.realized_r` directly (the "n=340/341, WR 40%" book) understates true performance by ~23 points of WR | Pre-existing | HC-A3/HC-A4 |
